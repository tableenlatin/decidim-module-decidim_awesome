...
However you can force some specific version using `gem "decidim-decidim_awesome", "~> 0.32.0"` in the Gemfile.
...
By default this Gemfile resolves against the latest published Decidim `0.32.x`
release line available on RubyGems. If you need to test against a local Decidim
checkout instead, set `DECIDIM_PATH=/absolute/path/to/decidim` before running
Bundler or the test suite.
