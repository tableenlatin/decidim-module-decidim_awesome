  ...
  module Admin
    describe ChecksController do
      ...
      describe "awesome version checking" do
        let(:current_version) { Decidim::DecidimAwesome::VERSION }
        let(:current_minor) { current_version.split(".")[0..1].join(".") }
        let(:github_releases) do
          [
            { "tag_name" => "v#{current_minor}.2", "draft" => false, "prerelease" => false },
            { "tag_name" => "v#{current_minor}.1", "draft" => false, "prerelease" => false },
        ...
        before do
          ...
          allow(controller.helpers).to receive(:awesome_version).and_return(current_version)
        end
        ...
        describe "#awesome_latest_version" do
          context "when GitHub API returns releases" do
            ...
            it "returns the latest version for the current minor" do
              expect(controller.helpers.awesome_latest_version).to eq("#{current_minor}.2")
            end

            it "filters out drafts and prereleases" do
              releases_with_draft = github_releases + [{ "tag_name" => "v#{current_minor}.3", "draft" => true, "prerelease" => false }]
            ...
            it "only checks current minor version" do
              releases_with_newer_minor = [{ "tag_name" => "v999.0.0", "draft" => false, "prerelease" => false }] + github_releases
        ...
        describe "#awesome_version_outdated?" do
          context "when latest version is available" do
            before do
              allow(controller.helpers).to receive(:awesome_version).and_return("#{current_minor}.1")
              allow(controller.helpers).to receive(:awesome_latest_version).and_return("#{current_minor}.2")
            end
      ...
      describe "decidim version checking" do
        let(:current_decidim_version) { Decidim.version }
        let(:current_decidim_minor) { current_decidim_version.split(".")[0..1].join(".") }
        let(:decidim_releases) do
          [
            { "tag_name" => "v#{current_decidim_minor}.2", "draft" => false, "prerelease" => false },
            { "tag_name" => "v#{current_decidim_minor}.1", "draft" => false, "prerelease" => false },
            { "tag_name" => "v0.0.1", "draft" => false, "prerelease" => false }
          ]
        end
        ...
        before do
          ...
          allow(controller.helpers).to receive(:decidim_version).and_return(current_decidim_version)
        end
        ...
        describe "#decidim_latest_version" do
          context "when GitHub API returns releases" do
            ...
            it "returns the latest version for the current minor" do
              expect(controller.helpers.decidim_latest_version).to eq("#{current_decidim_minor}.2")
            end

            it "filters out drafts and prereleases" do
              releases_with_draft = decidim_releases + [{ "tag_name" => "v#{current_decidim_minor}.3", "draft" => true, "prerelease" => false }]
              allow(faraday_response).to receive(:body).and_return(releases_with_draft.to_json)
            ...
            it "only checks current minor version" do
              releases_with_newer_minor = [{ "tag_name" => "v999.0.0", "draft" => false, "prerelease" => false }] + decidim_releases
              allow(faraday_response).to receive(:body).and_return(releases_with_newer_minor.to_json)
        ...
        describe "#decidim_version_outdated?" do
          context "when latest version is available" do
            before do
              allow(controller.helpers).to receive(:decidim_version).and_return("#{current_decidim_minor}.1")
              allow(controller.helpers).to receive(:decidim_latest_version).and_return("#{current_decidim_minor}.2")
            end
