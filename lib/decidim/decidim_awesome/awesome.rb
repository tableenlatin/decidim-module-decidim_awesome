...
module Decidim
  module DecidimAwesome
    ...
    # The config backing — `ActiveSupport::OrderedOptions` is a Hash with
    # method-style access, so it supports every usage the previous
    # `ActiveSupport::Configurable` API exposed:
    #   - hash-style:    DecidimAwesome.config[:foo], .keys, .to_h, .except!
    #   - method-style:  DecidimAwesome.config.foo, .foo = value
    # The `method_missing` below adds the third style we relied on:
    #   - direct:        DecidimAwesome.foo, DecidimAwesome.foo = value
    # so the rest of the codebase (engine, models, permissions, …) needs no
    # change. ActiveSupport::Configurable is deprecated in Rails 8.1 and
    # removed in 8.2, hence the migration.
    def self.config
    ...
    config.allow_images_in_editors = false
    ...
    config.intergram_for_admins = true

    # In the public side only
    config.intergram_for_public = true
    ...
    config.validate_title_min_length = 15
    ...
    config.weighted_proposal_voting = true
    ...
    config.votes_by_proposal_status = true
    ...
    config.allow_limiting_amendments = false
    ...
    config.hashcash_signup_bits = 20
    config.hashcash_login_bits = 16
    ...
    config.proposal_private_custom_fields = {}
    ...
    config.force_authorization_allowed_controller_names = %w(account)
    ...
    config.menu = []
    config.mobile_menu = []
    config.home_content_block_menu = []
    ...
    config.intergram_url = "https://www.intergram.xyz/js/widget.js"

    # no need to override these settings, there admin-configurable
    config.intergram_for_admins_settings = {
      chat_id: nil,
      color: nil,
    ...
    config.intergram_for_public_settings = {
      chat_id: nil,
      require_login: true,
      color: nil,
      use_floating_button: false,
    ...
    config.participatory_spaces_routes_context = {
    ...
    config.voting_components = [:proposals, :reporting_proposals]
